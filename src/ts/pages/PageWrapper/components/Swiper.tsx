...
import { COMMON, PERSON } from '../helpers/menu';
...
function addSwipeEvents(
  ...
  function onEnd() {
    ...
    const list  = type === 'common' ? COMMON : PERSON;
    const pages = list.map((item) => item.id).filter((value) => value);
    const index = pages.indexOf(page || 'total');
    const getUrl = type === 'common'
      ? (newPage?: string) => `/common/${newPage || 'total'}`
      : (newPage?: string) => `/person/${newPage || 'month'}/${userId || '0'}`;
